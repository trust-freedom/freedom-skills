...
- 视频：`{base_name}.mp4`
- 音频：`{base_name}.mp3`
- 分段转录：`{base_name}-chunks/{base_name}-0000.txt`、`{base_name}-chunks/{base_name}-0001.txt`、...（每个分段一个文件，与分段音频同目录）
- 汇总转录：`{base_name}.txt`（所有分段合并后的完整转录）
- 转录纠错：`{base_name}-correct.txt`（AI 纠错后）
- 文档：`{base_name}.md`
- 临时分段文件：`{base_name}-chunks/` 目录，包含分段音频和分段 txt（可手动清理）
...
2. 用 faster-whisper（large-v3 模型，int8 量化）逐段转录，通过 `initial_prompt` 引导输出中文标点
3. 每个 VAD segment 输出为一行（约一句话），停顿 ≥ 0.5 秒自动插入空行分段，连续 8 行无分段时强制分段
...
2. 如果未完成，统计 `{base_name}-chunks/{base_name}-NNNN.txt` 已完成的数量，报告进度（如 "8/15 分段已完成"）
...
- 转录输出包含中文标点（通过 `initial_prompt` 引导），每个 segment 一行，停顿 ≥ 0.5 秒或连续 8 行自动分段
- 支持断点续传：中断后重新运行相同命令，每个分段独立检测完成状态，自动跳过已完成的分段
- `--workers` 控制并行转录进程数（默认 1，推荐 2），每个进程独立加载模型
- 临时分段音频和分段 txt 保存在 `{base_name}-chunks/` 目录，转录完成后可手动删除
